/**
 * PDF Export Service - Unified export layer
 * ALL exports now use HTML mode: Edge functions return Gemini-generated HTML,
 * converted to PDF client-side via iframe print dialog.
 */
...
function buildFileName(type: ExportType, entityName?: string): string {
  ...
  switch (type) {
    case "tasks": return `QUADRO_DE_TAREFAS_${dateStr}_${code}.pdf`;
    case "project": return `PROJETO_${slugify(entityName || "PROJETO")}_${dateStr}_${code}.pdf`;
    case "report_360": return `RELATORIO_360_${dateStr}_${code}.pdf`;
    case "finance": return `RELATORIO_FINANCEIRO_${dateStr}_${code}.pdf`;
    case "project_overview": return `VISAO_GERAL_PROJETOS_${dateStr}_${code}.pdf`;
    case "portal": return `PORTAL_CLIENTE_${slugify(entityName || "PORTAL")}_${dateStr}_${code}.pdf`;
    case "campaign": return `CAMPANHA_${slugify(entityName || "CAMPANHA")}_${dateStr}_${code}.pdf`;
    case "content": return `CONTEUDO_${slugify(entityName || "CONTEUDO")}_${dateStr}_${code}.pdf`;
    case "creative": return `CRIATIVO_${slugify(entityName || "CRIATIVO")}_${dateStr}_${code}.pdf`;
    default: return `HUB_EXPORT_${dateStr}_${code}.pdf`;
  }
}
...
async function convertHtmlToPdf(html: string, _fileName: string): Promise<void> {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Inject @media print if not present
      let finalHtml = html;
      if (!html.includes('@page')) {
        finalHtml = html.replace('</style>', `
...
async function exportPdf(
  type: ExportType,
  params: { id?: string; period?: string; token?: string; filters?: Record<string, unknown>; entityName?: string } = {},
  options: ExportOptions = {}
): Promise<ExportResult> {
  const opts = { ...DEFAULT_OPTIONS, ...options };
  let toastId: string | number | undefined;
  if (opts.showToasts) toastId = toast.loading(opts.loadingMessage);
  ...
  try {
    ...
    const { data, error } = await supabase.functions.invoke<ExportResult>("export-pdf", {
      body: { type, id: params.id, period: params.period, token: params.token, filters: params.filters },
    });
    ...
    if (data.html) {
      ...
      if (opts.showToasts && toastId) toast.success(opts.successMessage, { id: toastId });
      return { success: true };
    }

    // Fallback: URL-based download
    const url = data.signed_url || data.public_url;
    if (url) await downloadPdfFromUrl(url, fileName);
    if (opts.showToasts && toastId) toast.success(opts.successMessage, { id: toastId });
    return data;
  } catch (error) {
    ...
    if (opts.showToasts && toastId) toast.error(errorMessage, { id: toastId });
    return { success: false, error: errorMessage };
  }
}

/**
 * Standalone function invoker - handles HTML or URL responses from dedicated edge functions.
 */
...
export async function exportFocusPDF(
  ...
  return invokeAndDownload(
    ...
    "tasks", "FOCO", "PDF do plano de foco pronto!", options
  );
}
