  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <span className="text-[10px] text-muted-foreground">{project.client.company}</span>
      ...
      <div className="flex items-center justify-between mt-2 pt-2 border-t border-border/30">
        ...
        <span className={`text-[9px] px-1.5 py-0.5 rounded border ${statusConfig.color}`}>
          {statusConfig.label}
        </span>
      </div>
    </div>
...
export function ProjectsDashboard() {
  ...
  const totalPipeline = projects.reduce((acc, p) => acc + p.contractValue, 0);
  const avgHealth = Math.round(projects.reduce((acc, p) => acc + p.healthScore, 0) / projects.length);
  const projectsInRisk = projects.filter(p => p.status === 'em_risco' || p.status === 'atrasado').length;
  const projectsBlocked = projects.filter(p => p.blockedByPayment).length;
  ...
  const topProjects = [...projects].sort((a, b) => b.contractValue - a.contractValue).slice(0, 4);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 pb-6 border-b border-border/50">
        ...
        <div className="glass-card rounded-2xl p-4 flex items-center gap-4">
          <div>
            <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-wider mb-1">Meta Mensal</p>
            <p className="text-lg font-bold text-foreground">
              {formatCurrency(284000)} <span className="text-sm text-muted-foreground">/ {formatCurrency(400000)}</span>
            </p>
          </div>
          <RadialProgress value={71} size={60} label="META" />
        </div>
      </div>

      {/* Main Grid */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        {/* Left Column - Visual Board + Timeline */}
                      ...
                      <div className="h-1.5 bg-muted rounded-full">
                        <div 
                          className={`h-full rounded-full ${
                            p.healthScore > 90 ? 'bg-emerald-500' : 
                            p.healthScore > 70 ? 'bg-primary' : 'bg-amber-500'
                          }`}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-foreground">92%</span>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-muted-foreground">Cloud Storage 10Gbps</span>
                  <div className="flex items-center gap-2">
                    <div className="w-20 h-1.5 bg-muted rounded-full overflow-hidden">
        ...
        <div className="xl:col-span-4 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="glass-card rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[9px] text-emerald-500 font-bold">+24.5%</span>
              </div>
            ...
            <div className="glass-card rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[9px] text-primary font-bold">92% On-time</span>
              </div>
              <p className="text-[9px] text-muted-foreground uppercase tracking-wider mb-1">Projetos em Workflow</p>
            ...
            <div className="glass-card rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[9px] text-amber-500 font-bold">High Perf</span>
              </div>
              <p className="text-[9px] text-muted-foreground uppercase tracking-wider mb-1">Eficiência Squad</p>
            ...
            <div className="glass-card rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[9px] text-emerald-500 font-bold">Financial OK</span>
              </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p className="text-[10px] text-primary font-bold">{project.client.company}</p>
          ...
          <div className="rounded-3xl p-5 bg-primary/5 border border-primary/10">
            ...
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              "Gargalo detectado em {projects[0]?.id.toUpperCase() || 'SF-092'}. Recomendo escalar editor assistente para manter SLA."
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="glass-card rounded-2xl p-4">
          ...
          <p className="text-lg font-bold text-foreground">{formatCurrency(stats.valorTotal)}</p>
        ...
        <div className="glass-card rounded-2xl p-4">
          ...
          <p className="text-lg font-bold text-foreground">{stats.healthMedia}%</p>
        ...
        <div className="glass-card rounded-2xl p-4">
          ...
          <p className="text-lg font-bold text-foreground">{stats.ok}</p>
        ...
        <div className="glass-card rounded-2xl p-4">
          ...
          <p className="text-lg font-bold text-foreground">{stats.emRisco}</p>
