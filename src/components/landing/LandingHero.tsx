...
export function LandingHero({ scrollContainerRef }: LandingHeroProps) {
  ...
  const textOpacity = useSpring(useTransform(scrollYProgress, [0, 0.5], [1, 0]), { stiffness: 120, damping: 30 });
  const textY = useSpring(useTransform(scrollYProgress, [0, 0.5], [0, -60]), { stiffness: 120, damping: 30 });
  const videoY = useSpring(useTransform(scrollYProgress, [0, 1], [0, 120]), { stiffness: 120, damping: 30 });
  const videoScale = useSpring(useTransform(scrollYProgress, [0, 0.5], [1, 1.05]), { stiffness: 120, damping: 30 });

  return (
    <section ref={sectionRef} className="relative z-10 px-6 md:px-12 pt-32 pb-12 md:pt-44 md:pb-28 min-h-screen flex items-center">
      <div className="max-w-7xl mx-auto w-full">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          {/* Text */}
          <motion.div className="space-y-8" style={{ opacity: textOpacity, y: textY }}>
            <motion.div
            ...
            >
              <Button size="lg" onClick={() => navigate('/login')} className="gap-2 bg-primary hover:bg-primary/90 px-8 h-12 hover-invert">
                Começar Agora <ArrowRight className="w-4 h-4" />
              </Button>
              <Button size="lg" variant="outline" className="gap-2 border-border/50 hover:border-primary/20 hover:bg-primary/5 h-12 px-8">
